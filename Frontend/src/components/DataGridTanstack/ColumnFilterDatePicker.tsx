...
export function makeColumnFilterDatePicker<T extends object>(configs: CreateDataGridConfig<T>) {
    ...
    const ColumnFilterDatePicker = <T extends any>({columnId, index}: ColumnFilterNumericProps<T>) => {
        ...
        const currentMin = columnFilterValue?.[0] ? new Date(columnFilterValue?.[0]).toISOString().slice(0, 10) : null;
        const currentMax = columnFilterValue?.[1] ? new Date(columnFilterValue?.[1] ?? "").toISOString().slice(0, 10) : null;

        const handleOnChange = (type: "min" | "max") =>
            (valueDayjs: Dayjs | null) => {
